...
int NON_SPEECH_TOKENS[] = {1,     2,     7,     8,     9,     10,    14,    25,    26,    27,    28,    29,    31,
                           58,    59,    60,    61,    62,    63,    90,    91,    92,    93,    357,   366,   438,
                           532,   685,   705,   796,   930,   1058,  1220,  1267,  1279,  1303,  1343,  1377,  1391,
                           1635,  1782,  1875,  2162,  2361,  2488,  3467,  4008,  4211,  4600,  4808,  5299,  5855,
                           6329,  7203,  9609,  9959,  10563, 10786, 11420, 11709, 11907, 13163, 13697, 13700, 14808,
                           15306, 16410, 16791, 17992, 19203, 19510, 20724, 22305, 22935, 27007, 30109, 30420, 33409,
                           34949, 40283, 40493, 40549, 47282, 49146, 50257, 50357, 50358, 50359, 50360, 50361};

using json = nlohmann::json;

PostProcModel::PostProcModel(const string& token_file) : TFLiteModel("post_proc") { _token_file = token_file; }

bool PostProcModel::initialize(string model_file, string lib_dir, string cache_dir, int backend) {
...
void PostProcModel::invoke(bool measure_time) { TFLiteModel::invoke(measure_time); }

void PostProcModel::apply_timestamp_rules(float* logits, int logits_size, vector<int>& tokens) {
    logits[TOKEN_NO_TIMESTAMP] = -1e9;
    bool last_was_timestamp = (tokens.size() >= 2 && tokens[tokens.size() - 1] >= TOKEN_TIMESTAMP_BEGIN);
    bool penultimate_was_timestamp = (tokens.size() < 3 || tokens[tokens.size() - 2] >= TOKEN_TIMESTAMP_BEGIN);

    if (last_was_timestamp) {
        if (penultimate_was_timestamp)  // has to be non-timestamp
            LOGITS_TO_NEG_INF(&logits[TOKEN_TIMESTAMP_BEGIN], &logits[logits_size])
        else  // cannot be normal text tokens
            LOGITS_TO_NEG_INF(logits, &logits[TOKEN_EOT])
    }

    vector<int> timestamps;
    int timestamp_last;
    for (auto& token : tokens) {
        if (token >= TOKEN_TIMESTAMP_BEGIN) timestamps.push_back(token);
    }
    if (timestamps.size() > 0) {
        ...
        LOGITS_TO_NEG_INF(&logits[TOKEN_TIMESTAMP_BEGIN], &logits[timestamp_last])
    }

    if (tokens.size() == SAMPLE_BEGIN) {
        LOGITS_TO_NEG_INF(logits, &logits[TOKEN_TIMESTAMP_BEGIN])
...
int PostProcModel::process(int idx, float* logits, int logits_size, vector<int>& decoded_tokens, float base_timestamp) {
    ...
    for (auto& token : NON_SPEECH_TOKENS) {
        logits[token] = -1e9;
    }
...
void PostProcModel::proc_token(int token, float base_timestamp) {
    ...
    if (token == TOKEN_BLANK)
        ...
        if (start != string::npos && end != string::npos && end > start) {
            ...
            // this is optional. Whisper encoder doesn't support timestamps beyond
