...
constexpr const char* ARGMAX_WHISPERKIT_BUNDLE_INFO = "com.argmaxinc.whisperax";
...
class TFLiteMessenger {
   public:
    TFLiteMessenger() { _msg = std::make_unique<std::string>(); }
    std::mutex _mutex;
    ...
    ~TFLiteMessenger() { _msg.reset(); }

    std::string get_message() { return std::string(*(_msg.get())); }
